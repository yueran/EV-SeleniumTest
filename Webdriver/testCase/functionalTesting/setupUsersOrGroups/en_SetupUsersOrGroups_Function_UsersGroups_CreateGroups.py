...
class EnSetupUsersOrGroupsFunctionUsersGroupsCreateGroups(unittest.TestCase):
    ...
    def test_en_setup_users_or_groups_function_users_groups_create_groups(self):
        ...
        driver.find_element_by_id("userGroupName").send_keys(setupUsersOrGroupsNewUserGroupSendKey)
        driver.find_element_by_id("okUserGroupOptions").click()
        self.driver.implicitly_wait(30)
        try: self.assertIn(setupUsersOrGroupsNewUserGroup,driver.find_element_by_class_name("userGroupColumn").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        driver.refresh()
        try: self.assertIn(setupUsersOrGroupsNewUserGroup,driver.find_element_by_class_name("userGroupColumn").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
