...
class EnTemplateStyleFunctionModifyTmpName(unittest.TestCase):
    ...
    def test_en_template_style_function_modify_tmp_name(self):
        ...
        try: self.assertNotIn(templateStylesTmpNameModified, driver.find_element_by_id("templateSpace").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        ...
        driver.find_element_by_id("newName").send_keys(templateStylesTmpNameModified)
        driver.find_element_by_id("rename").click()

        driver.find_element_by_css_selector("div.bigDownArrow").click()
        for i in range(60):
            try:
                if templateStylesTmpNameModified == driver.find_element_by_css_selector("#templates_template"+templateStylesModifyTmpID+"> div.itemContent.templateBg > div.blockText").text: break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        try: self.assertNotIn(templateStylesModifyTmp, driver.find_element_by_id("templateSpace").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        try: self.assertIn(templateStylesTmpNameModified, driver.find_element_by_id("templateSpace").text)
        except AssertionError as e: self.verificationErrors.append(str(e))
        driver.refresh()
        driver.find_element_by_css_selector("div.bigDownArrow").click()
        for i in range(60):
